...
async function getForUser(username) {
  return PizzaStore.findAll({
    ...
    raw: true, // tells seuqlize to return raw data instead of model instance.
  }).then(debriefPizzas);
}
...
// toppings is an array, and will throw an error if we try to save it in the db
// column that accepts only type String.
// So create two methods to turn the toppings array into JSON and back again.
...
// queries to sequelize will return an array of pizzas, this
// method will help us process the array to turn toppings back
// to the right format.
